...
# Command namespace for the wake prompt. Claude plugin commands are
# /autocoder:-prefixed; every other agent's command tree (gemini extension,
# .agent workflows) uses bare names (/manager-resume, /monitor-loop).
wake_cmd_prefix() {
  case "$AGENT_NAME" in
    claude) printf 'autocoder:' ;;
    *)      printf '' ;;
  esac
}
