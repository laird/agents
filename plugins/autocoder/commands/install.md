...
if [ "$USER_APPROVED_SCRIPTS" = "yes" ]; then
  ...
  # Install the six terminal commands. HOW depends on where the scripts live:
  #
  # - Repo checkout: symlink. The path is stable and tracks git pulls.
  # - Plugin cache: generate a wrapper that resolves the CURRENT plugin
  #   version at RUN time. NEVER symlink into the cache: every plugin update
  #   installs into a NEW versioned directory (.../autocoder/<ver>/scripts),
  #   so a symlink pins whatever version was current at install time and
  #   silently runs stale code after every update (a start-parallel pinned
  #   to 4.21.0 kept picking tmux because herdr support shipped in 4.22.0).
  install_cli() {
...
#!/bin/bash
# Generated by /autocoder:install — do not edit; rerun /autocoder:install.
# Resolves the CURRENT autocoder plugin version at run time so plugin
# updates take effect without reinstalling these commands.
DIR=\$(python3 -c '
import json, os
try:
    data = json.load(open(os.path.expanduser("~/.claude/plugins/installed_plugins.json")))
    for key, entries in data.get("plugins", {}).items():
        if key.split("@")[0] == "autocoder" and entries:
            print(os.path.join(entries[0]["installPath"], "scripts")); break
except Exception:
    pass' 2>/dev/null)
if [ -z "\$DIR" ] || [ ! -f "\$DIR/$script" ]; then
  DIR=\$(find "\$HOME/.claude/plugins/cache" -maxdepth 5 -type d -path "*/autocoder/*/scripts" 2>/dev/null | sort -V | tail -1)
fi
if [ -z "\$DIR" ] || [ ! -f "\$DIR/$script" ]; then
  echo "❌ Could not locate the installed autocoder plugin scripts" >&2
  exit 1
fi
...
WRAPPER
        ...
        ;;
      *)
        ln -sf "$SCRIPT_DIR/$script" "$INSTALL_DIR/$name"
        ;;
    esac
  }

  install_cli start-parallel start-parallel-agents.sh
  install_cli add-worker add-worker.sh
  install_cli remove-worker remove-worker.sh
  install_cli join-parallel join-parallel-agents.sh
  install_cli end-parallel end-parallel-agents.sh
  install_cli stop-parallel stop-parallel-agents.sh
