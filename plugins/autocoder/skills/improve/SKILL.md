...
in production on a multi-stage LLM research platform: 12 rounds in a single day surfaced and fixed
11+ distinct defect classes.
...
1. Read the target repo's `CLAUDE.md` for a validation binding — conventionally a
   "Validation & Refinement Loop" section naming the full-cycle command with its exact
   flags, the worker singleton restart procedure, log locations, and reconciliation tooling.
...
1. **Deploy exactly what you think you're testing.** Rebuild, then restart the serving
   ...
   SIGKILL. This single discipline has caught two otherwise-invisible corrupted rounds.
...
3. **Grade against an executable contract, not vibes.** Encode the operator's definition of "a
   successful run" as a checklist the run tool itself evaluates and exits non-zero on (e.g.
   five stages: inputs auto-approved, stress tests ≥ the configured minimum, deliverable
   completed, audited, findings addressed). Then grade beyond the checklist:
   ...
   - **Ground truth reconciliation**: compare the system's account of itself against external
     reality where APIs exist (provider billing/usage APIs vs internal cost tracking has
     exposed a 4-5× undercount). The daily delta becomes a standing regression test.
...
5. **Log every run, mandatory — richly enough to write the final report from the log alone.**
   ...
   classes × fixes. The reference shape: per-round chapters, then a closing verdict and a
   what-we-learned-each-round retrospective table. If the log can't
...
- Warm caches make reruns nearly free — budget systems must not count cache hits against
  live-call quotas (this exact bug has starved a first run outright).
...
a backlog (one stress-test fix took three attempts across three seams; serial
...
truth. Write them up under a `CLAUDE.md` §"Validation & Refinement Loop" heading so the next
session finds the binding where this skill looks for it.
