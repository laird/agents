...
ln -sfn "$AGENTS_REPO_ROOT/plugins/autocoder/scripts/add-worker.sh" "$BIN_DIR/add-worker"
ln -sfn "$AGENTS_REPO_ROOT/plugins/autocoder/scripts/remove-worker.sh" "$BIN_DIR/remove-worker"
...
echo "  4. Use /dev, /assess, /plan, etc. as slash commands"
