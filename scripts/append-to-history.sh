...
**Impact**: ${IMPACT}"
  ...
  if [[ "$HISTORY_FILE" != */* ]]; then
    # See plugins/autocoder/scripts/issue-config.sh for why this is two steps:
    # piping git straight into grep -m1 races git's own write completion
    # under a caller's pipefail.
    _ath_WORKTREE_LIST=$(git worktree list --porcelain 2>/dev/null)
    MAIN_WT=$(printf '%s\n' "$_ath_WORKTREE_LIST" | grep -m1 '^worktree ' | sed 's/^worktree //')
