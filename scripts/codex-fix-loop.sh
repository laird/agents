#!/bin/bash
# Run the Codex autocoder workflow repeatedly using shell control instead of Claude hooks.
...
while true; do
  if [ -f "$STOP_FILE" ]; then
    echo "🛑 Stop file detected, ending Codex fix loop"
    break
  fi
  ...
  OUTPUT_FILE=$(mktemp)
  if [ -n "$RUN_ISSUE_NUMBER" ]; then
    bash "$SCRIPT_DIR/codex-autocoder.sh" fix "$RUN_ISSUE_NUMBER" | tee "$OUTPUT_FILE"
  else
    bash "$SCRIPT_DIR/codex-autocoder.sh" fix | tee "$OUTPUT_FILE"
  fi

  if grep -q '^IDLE_NO_WORK_AVAILABLE$' "$OUTPUT_FILE"; then
    set_status "idle"
    echo "CODEX_FIX_LOOP_IDLE"
