#!/bin/bash
# Run the Gemini (Antigravity) worker monitor on a recurring interval.
...
while true; do
  if [ -f "$STOP_FILE" ]; then
    echo "🛑 Stop file detected, ending Gemini monitor loop"
    break
  fi
  bash "$SCRIPT_DIR/gemini-autocoder.sh" monitor-workers
  sleep "$((INTERVAL_MINUTES * 60))"
done

echo "✅ Gemini monitor loop stopped"
