# Start Infinite Dev-GitHub Loop

Wrapper around `/autocoder:dev` that runs it in a loop forever.
...
<!-- BEGIN optional-skills-mapping dev-loop v1 — keep in sync between Claude/Antigravity mirrors of this command -->

`/dev-loop` is a dispatcher coordinating parallel workers. The dispatcher and workers have distinct skill mappings.
...
**Per-worker level** (each worker in its own worktree; workers receive the dispatcher's manifest and apply the full `/dev` mapping above to their assigned issue, including the same composition rule and ordering-across-steps rule), plus:

| Step | Skill mapping |
|---|---|
| Each worker finishes its branch (PR / merge) | `superpowers:finishing-a-development-branch` |

**Worker dispatch.** Prepend the `optional-skills-manifest v1` block (from `plugins/shared/optional-skills-prelude.md` / `.agent/shared/optional-skills-prelude.md`) as the **first paragraph** of each worker's prompt. Generate the bullet list as the intersection of your available skills × the per-worker skill set: `peters-toolkit:bugfix`, `superpowers:systematic-debugging`, `superpowers:test-driven-development`, `superpowers:using-git-worktrees`, `superpowers:verification-before-completion`, `superpowers:requesting-code-review`, `superpowers:receiving-code-review`, `superpowers:finishing-a-development-branch`, `thorough-brainstorming`, `thorough-writing-plans`, `superpowers:brainstorming`, `superpowers:writing-plans`, `autocoder:improve-test-coverage`, `critical-design-review`, `critical-implementation-review`, `completion-review`.

**Worker autonomy.** Workers run unattended. Instruct each worker that it must never ask a question or wait for an in-chat approval: when `peters-toolkit:bugfix` is used, its G1/G2/G4/G5/G8 gates resolve per the *Autonomous gate policy* in `/dev`'s mapping block — self-decide and record the decision on the issue, and escalate with a blocking label (`needs-clarification`, `needs-design`, `needs-approval`, `too-complex`) instead of pausing mid-issue. The skill's process laws (failing reproduction test before the fix, no under-tiering, full verification before integration) still bind.

<!-- END optional-skills-mapping dev-loop v1 -->
...
2. Uses the `loop` skill to schedule `/autocoder:dev` every `IDLE_SLEEP_MINUTES` minutes
...
3. Runs `/autocoder:dev`
4. When Claude tries to exit, stop hook feeds `/autocoder:dev` back as input
5. Loop continues until manually stopped or max iterations reached

## Multi-Agent Coordination (Automatic)

When `CLAUDE_CODE_TASK_LIST_ID` is set, `/dev-loop` automatically coordinates with other agents:
...
# 3. Run /dev-loop in each worktree (same command everywhere)
# Main worktree
cd /path/to/project
/dev-loop  # → Auto-detects: "I'm coordinator, will deploy when ready"

# Feature worktree 1
cd /path/to/project-wt-auth
/dev-loop  # → Auto-detects: "I'm worker, will complete tasks"
...
- Same `/dev-loop` command everywhere
...
The stop hook will automatically re-invoke `${LOOP_TARGET}` when Claude exits, creating an infinite loop. The gate dispatches to `/autocoder:dev` only when there is work to do (spec §5.4).
