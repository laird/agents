...
if [ -f "GEMINI.md" ]; then
    ...
    if grep -q "### Unit Tests Only" GEMINI.md; then
    ...
    if grep -q "Working directory: " GEMINI.md && grep -A5 "### Unit Tests Only" GEMINI.md | grep -q "Working directory:"; then
        UNIT_TEST_DIR=$(grep -A5 "### Unit Tests Only" GEMINI.md | grep "Working directory:" | sed 's/.*Working directory: *`\([^`]*\)`.*/\1/' | head -1)
    else
        UNIT_TEST_DIR="."
    fi
