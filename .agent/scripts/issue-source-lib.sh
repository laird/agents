...
resolve_effective_issue_source() {
  ...
  if [ -n "$cli_source" ]; then
    case "$cli_source" in
      file|github|jira|ado) ;;
      *)
        echo "❌ Invalid --issue-source '$cli_source'. Use file, github, jira, or ado." >&2
        return 2
        ;;
    esac
  ...
  if [ "$ISSUE_SOURCE" != "file" ] && [ "$ISSUE_SOURCE" != "github" ] && [ "$ISSUE_SOURCE" != "jira" ] && [ "$ISSUE_SOURCE" != "ado" ]; then
