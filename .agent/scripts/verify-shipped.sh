...
#   explicit arg > CLAUDE_CODE_SHIP_BRANCH > GEMINI.md > repo default > main
#
# The GEMINI.md step matters because headless workers (cron, tmux panes started
...
if [ -z "$SHIP_BRANCH" ]; then
    for cfg in GEMINI.md gemini.md AGENTS.md; do
        [ -f "$cfg" ] || continue
