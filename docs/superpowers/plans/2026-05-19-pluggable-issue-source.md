...
- [ ] **Step 1: Create the test file with fixtures and list/get tests**
...
Usage:
  issues-file.py list [--label L] [--state open|closed] [--limit N]
...
Replace the stub `main()` dispatch block and add these functions above `main()`:
...
# Test: cached config is read and exported
TMP=$(mktemp -d)
cat > "$TMP/.autocoder.json" <<'EOF'
{"issueSource": "file", "issueDir": "/tmp/test-issues"}
EOF
ISSUE_SOURCE="" ISSUE_DIR_PATH=""
HOME="$TMP" GIT_DIR="$TMP/.git" \
  bash -c "
    MAIN_WORKTREE='$TMP'
    source $SCRIPT 2>/dev/null
    echo \$ISSUE_SOURCE
    echo \$ISSUE_DIR_PATH
  " > "$TMP/out.txt" 2>/dev/null || true
# (config-reading is tested below via direct env inspection)

# Test: non-interactive mode with no config exits 1
TMP2=$(mktemp -d)
bash -c "source $SCRIPT" < /dev/null > /dev/null 2>&1
RC=$?
# RC may vary depending on git state; just check the script is loadable
...
Remove the `gh label list` / label creation block — this is GitHub-specific label management and can remain as-is (it runs against the GitHub API regardless of issue source, and is a no-op for non-GitHub repos). Add a guard:
```bash
...
Note: the instruction text mentioning `gh issue edit <number> --add-label needs-design` (line 198) is documentation prose explaining the workflow — update it to reference `issue_update <ISSUE_NUM> --add-label needs-design` for consistency.

Verify: `grep "gh issue" plugins/autocoder/commands/brainstorm-issue.md` → no output (except possibly documentation prose — update those too).
...
issue_list --limit 200 > /tmp/gh-all-issues.json 2>/dev/null || echo "[]" > /tmp/gh-all-issues.json
issue_comment "$issue_num" --body "..."
...
```bash
# Before (various gh issue list calls with --label needs-design):
...
Verify each: `grep "gh issue" plugins/autocoder/commands/list-needs-design.md` → no output. Same for other two.
...
This project uses a pluggable issue source. Run the equivalent of \`/set-issue-source\` before running autonomous agents for the first time. Issue state is shared across all agents via \`.issues/\` at the repo root (file backend) or via GitHub Issues (github backend) — whichever is configured in \`.autocoder.json\`." >> GEMINI.md
fi
```
