...
│   └── harden/                          # Platform-neutral harden skill (all platforms)
...
- Per-platform directories (`.agent/`, `.factory/`, `codex-plugins/`) mirror the Claude Code `plugins/` content for their respective agent CLIs.
- `skills/harden/` is platform-neutral — it ships once and is referenced by all platforms.
