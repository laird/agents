...
assert_eq "$AGENT_LAUNCH_CMD" "codex --dangerously-bypass-approvals-and-sandbox" "Codex launch command"
assert_eq "$MANAGER_LAUNCH_CMD" "codex --dangerously-bypass-approvals-and-sandbox" "Codex manager launch command"

CODEX_AUTOCODER="$SCRIPT_DIR/../../../scripts/codex-autocoder.sh"
CODEX_MONITOR="$SCRIPT_DIR/../../../scripts/codex-monitor-workers.sh"
assert_eq "$(grep -c 'codex exec --dangerously-bypass-approvals-and-sandbox' "$CODEX_AUTOCODER")" "2" "Both Codex autocoder branches bypass approvals and sandbox"
assert_eq "$(grep -c 'codex exec --dangerously-bypass-approvals-and-sandbox' "$CODEX_MONITOR")" "1" "Codex manager monitor bypasses approvals and sandbox"
