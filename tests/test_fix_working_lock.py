"""Guards the 'working' claim-lock lifecycle in fix.md (issue #14).

Honest scope note
-----------------
fix.md is a slash-command body: the lock rules are **prose the LLM runtime
...
So these tests assert on the text itself, and are deliberately narrow:
  ...
  * every remaining `--remove-label "working"` sits next to a terminal outcome
    (a close, an explicit release comment, an awaiting-integration hand-off,
    or a failed-claim abort);
  * the Claude and Antigravity mirrors say the same thing.

If fix.md ever grows a real bash release function, replace these with
functional tests.
"""
...
def test_every_release_site_is_terminal(path):
    """Each remaining release must sit beside a close, an explicit release,
    an awaiting-integration hand-off, or a failed claim.
    ...
    terminal_markers = (
        ...
        "release comment",
        "releasing claim",      # explicit release comment (skip path)
        "add-blocking-label",   # blocking path releases for us
        "awaiting-integration", # PR hand-off: lock swapped, issue stays unclaimable
        "Releasing and aborting",     # claim race lost — claim never started
        "Releasing and trying next",  # claim race lost — claim never started
        "refusing to start",          # fetch failed before any work began
        "Could not create or switch", # branch switch failed — claim never started
        "git pull --rebase origin",   # rebase-abort path — claim never started
        "Release the parent issue claim",  # decomposition hand-off to sub-tasks
...
def test_mirrors_agree_on_lock_section():
    ...
    assert claude == antigravity, (
        "fix.md lock sections have drifted between plugins/autocoder/commands/ "
        "and .agent/workflows/"
    )
