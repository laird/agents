...
Plugins live in `plugins/` (Claude Code) and are mirrored per platform in `.agent/` (Antigravity), `.factory/` (Droid), and `codex-plugins/` (Codex). Runtime scripts are in `plugins/autocoder/scripts/`. Skills are in `skills/`. Per-platform marketplace config is in `.claude-plugin/`, `.factory-plugin/`, etc.
